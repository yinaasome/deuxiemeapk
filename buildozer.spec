...
requirements = python3,kivy==2.3.1,sqlite3,pandas,matplotlib,numpy,pillow,plyer,openpyxl,cython,javac
...
# (bool) Android logcat only display log for tags in the filter list.
android.logcat_filters = *:S python:D
...
# (str) XML file for Android Auto Backup (usable only if auto backup is enabled)
...
# (list) python-for-android whitelist of recipes to build against
# p4a.whitelist =
...
# --private the private directory of the app (default is current directory)
# --dir the directory to look for the app (default is current directory)
# Both are relative to the source.dir
# android.p4a_dir = 
# android.p4a_private = 
...
# Get a list of available identities: Xcode → Product → Archive → ... → Distribute App → Ad Hoc → Next → App Store Connect → Next → Select a certificate for code signing
